...
user_input = "What is the top speed of a leopard in kilometers per hour?"
...
def search_duckduckgo(query: str, num_results: int = 3):
    ...
    # the next line was needed for llama3.2:3b-instruct-fp16, llama3.1:8b-instruct-fp16
...
def llm_call_with_tools():
    ...
    if completion_1.choices[0].message.tool_calls:
        ...
        for tool_call in completion_1.choices[0].message.tool_calls:
            ...
            # args seems to be problematic with llama3.2:3b-instruct-fp16
            logger.info(f"Args:", args)
            # logger.info(f"Name: {name}, Num Results: {num_results} (Type: {type(num_results)})")
...
# --------------------------------------------------------------
def llm_call_structured_output_leopard(messages) -> LeopardSpeed:
...
if search_results_good:
    response = llm_call_structured_output_leopard(messages)
    logger.info("%s %d", user_input, response.topspeed)
else:
