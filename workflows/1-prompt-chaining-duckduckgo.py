...
def llm_call_structured_output_burr(messages) -> BurrSutter:

    completion_2 = client.beta.chat.completions.parse(
        model=MODEL_NAME,
        messages=messages,
        response_format=BurrSutter,
    )
...
if search_results_good:
    ...
    response = llm_call_structured_output_burr(messages)
    logger.info("%s\n%s", user_input, response.answer)
else:
